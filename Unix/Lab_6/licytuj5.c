...
int main(int argc, char **argv)
{
        ...
        key_t shmkey = ftok("/home/jbanasia/Dokumenty/licytuj5.c", 5);
        ...
        sem = sema_init(&(wspolna->semaphore),1,USYNC_PROCESS,0);
        ...
        for ( l = 0; l < N_AGENTS; l ++)
        {
                ...
                if (f == 0)
                {
                        for (j = 0; j < BIDDING_ROUNDS; j++)
                        {
                                ...
                                sema_wait(&(wspolna->semaphore));
                                Bids[N] = Bids[N] + R;
                                NBids[N] = NBids[N] + 1;
                                sema_post(&(wspolna->semaphore));
                        }
                exit(0);
                if(f < 0)
                {
        ...
        sema_destroy(&(wspolna->semaphore));
